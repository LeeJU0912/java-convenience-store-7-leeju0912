...
## 구현할 기능 목록
**[입력 단계]**
- 상품 목록, 행사 목록 로드 (파일 로드 예외)
- 입력 양식 = "[상품명-수량],[상품명-수량]" (양식 예외)

1. 프로모션 갯수 계산해보니, 공짜로 더 줄 수 있으면 출력 후 Y/N 선택
2. 공짜로 더 줄 갯수가 부족하면, 지금 산 거 그대로 결제할지, 정가 결제해야하는 물량 뺄지 선택
3. 멤버십 할인 받을지 말지 Y/N
4. 다 계산하고 제일 마지막에 한번 더 Y/N

- 총구매액 = 상품 가격 * 수량 (프로모션, 멤버십 할인)


- 잘못된 값을 입력하면 오류 출력 후 다시 진행
- 잘못된 값 = 4가지가 있음


**[계산 단계]**
- 고객이 상품을 구매하면, 즉시 재고에서 차감한다.

**[프로모션 기간]**
- 프로모션 기간에 N개 사면, 1개 추가로 준다. 각 상품마다 N값은 다르다.


**[멤버십 할인]**
- '프로모션 미적용' 의 30% 값 측정
- '프로모션 적용' 후 할인 연산 (음수가 될 수 있음)
- 최대 할인 가격 = 8000원
...
프로모션 기간이라면, v
1. 프로모션 전체 재고 확인 v
   프로모션 재고가 프로모션 혜택 이하인 경우, 물건 구매 (프로모션 -> 일반 물건 순) v

2. 내가 사는 양으로 프로모션 혜택 받는 재고 확인
3. Q1 프로모션 혜택 재고 알림 (프로모션 만큼 차감, 프로모션 만큼 영수증 기록) v
4. Q2 남은 양에 대해 알림 v
5. 물건 구매 (프로모션 만큼 차감, 프로모션 만큼 영수증 기록, 프로모션 기록)

프로모션 기간이 아니라면, v
1. 물건 구매 (프로모션 -> 일반 물건 순) v



**[물어볼 포인트]**
- Q1. 지금 2+1해서 2개 사셨는데 1개 더 가져오시면 공짜로 드려요.
- Q2. 프로모션 재고 적용할 물량이 다 떨어져서, 나머지 사는 건 다 정가로 사셔야 해요.
